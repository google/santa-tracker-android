...
class LaunchPenguinSwim(
    context: SantaContext,
    adapter: LauncherDataChangedCallback
) : AbstractFeatureModuleLaunch(context, adapter, R.string.swimming, R.drawable.android_game_cards_penguin_swim) {

    override val featureModuleNameId: Int = R.string.feature_penguin_swim
        ...
        when (state) {
            ...
            AbstractLaunch.STATE_FINISHED -> {
                val intent = SplashActivity.getIntent(
                        ...
                        titleRes,
                        featureModuleNameId,
                        R.color.penguin_swim_splash_screen_background,
                        false /* isLandscape */,
                        title,
                        imageView,
            ...
            AbstractLaunch.STATE_DISABLED -> {
                notify(mContext.applicationContext, getDisabledString(R.string.memory))
            }
    ...
    companion object {
        private val CLASS_NAME = "com.google.android.apps.santatracker.doodles.penguinswim.PenguinSwimActivity"
    }
}
